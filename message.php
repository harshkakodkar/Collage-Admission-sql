...
$courseNames = ["Science", "Commerce", "Arts"];
...
foreach ($courseNames as $course_name) {
    ...
    if ($result->num_rows == 1) {
        ...
        $stmtMessage = $conn->prepare("SELECT message FROM admission_messages WHERE student_id = ? AND course_name = ? ORDER BY message_id DESC LIMIT 1");
        $stmtMessage->bind_param("is", $studentID, $course_name);
        $stmtMessage->execute();
        $resultMessage = $stmtMessage->get_result();

        if ($resultMessage->num_rows == 1) {
            $rowMessage = $resultMessage->fetch_assoc();
            $courseMessages[$course_name] = $rowMessage['message'];
        } else {
        }

        $stmtMessage->close();
    } else {
        $courseMessages[$course_name] = "No student found";
    }
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-10 main-content">
                <h1>Messages</h1>
                <div>
                    <?php foreach ($courseMessages as $course_name => $message) : ?>
                        <p><strong><?php echo $course_name; ?>:</strong> <?php echo $message; ?></p>
                    <?php endforeach; ?>
                </div>
            </div>
        </div>
